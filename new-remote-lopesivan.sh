...
# Run!
project_name=$1
  user=$(git config --get github.lopesivan.user)
 token=$(git config --get github.lopesivan.token)
...
cat <<EOF >.clone
git clone git@github.com:\$(git config --get github.lopesivan.user)/${project_name}.git
EOF
git an
...
curl
  -X DELETE
  -H "Authorization: token \$(git config --get github.lopesivan.token)"
  https://api.github.com/repos/\$(git config --get github.lopesivan.user)/${project_name}
EOF
git an
...
echo
echo https://github.com/$(git config --get github.lopesivan.user)/${project_name}
# ----------------------------------------------------------------------------
exit 0
