...
# Run!
project_name=$1
  user=$(git config --get github.42algorithms.user)
 token=$(git config --get github.42algorithms.token)
...
cat <<EOF >.clone
git clone git@github.com:\$(git config --get github.42algorithms.user)/${project_name}.git
EOF
git an
...
curl
  -X DELETE
  -H "Authorization: token \$(git config --get github.42algorithms.token)"
  https://api.github.com/repos/\$(git config --get github.42algorithms.user)/${project_name}
EOF
git an
...
echo
echo https://github.com/$(git config --get github.42algorithms.user)/${project_name}
